    ...
    const simba_wstring& in_attributes)
{
        ...
        switch (in_requestType)
        {
            ...
            case ODBC_CONFIG_SYS_DSN:
            {
                    ...
                    (ODBC_ADD_SYS_DSN == in_requestType))
                {
                    ...
                    {
                        dsnConfig.Load(in_attributes, true);
                    }
                }
                else
                {
                ...
                // Pop the dialog if a parent window was supplied, otherwise operate in headless 
                // mode (ex. PowerShell).
                if ((NULL == in_parentWindow) ||
                    QSMainDialog::Show(in_parentWindow, dsnConfig))
                {
                    simba_string dsnStr = dsnConfig.GetDSN().GetAsUTF8();
                    driverLog->LogInfo(
                        "Simba::Quickstart",
    ...
    {
        driverLog->LogError("Simba::Quickstart", "ConfigDSN", "ConfigDSNInternal", driverError);
    ...
    catch (QSSetupException& ex)
    {
        driverLog->LogError(
            ...
            "ConfigDSNInternal", 
            "%s", 
            ex.GetErrorMsg().GetAsUTF8().c_str());
