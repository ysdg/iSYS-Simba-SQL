...
////////////////////////////////////////////////////////////////////////////////////////////////////
void QSDSNConfiguration::Load(const simba_wstring& in_attributeString, bool in_isNewDsn)
{
 #ifndef OLEDBTARGET
    m_logger->LogFunctionEntrance("Simba::Quickstart", "QSDSNConfiguration", "Load");

    m_configType = (in_isNewDsn) ? CT_NEW_DSN : CT_EXISTING_DSN;
        ...
        if (keyBufLen > 0)
        {
            ...
            while (*keyNamePtr)
            {
                ...
                *valBuf.Get() = '\0';
                ...
                // Don't override passed in attribute values.
                m_configuration.insert(make_pair(keyName, simba_wstring(valBuf.Get())));
...
void QSDSNConfiguration::Save()
{
    ...
    if (SQLWriteDSNToIniW(dsn.c_str(), driver.c_str()))
    {
        ...
        try
        {
            ...
            const simba_wstring& logSize = GetLogMaxSize();
            simba_uint32 size = (logSize.IsEmpty()) ? 0 : NumberConverter::ConvertWStringToUInt32(logSize);
        ...
        catch (QSSetupException& e)
        {
            m_logger->LogError(
                ...
                "Save",
                "%s",
                e.GetErrorMsg().GetAsUTF8().c_str());

            UWORD configMode;

            if (!SQLGetConfigMode(&configMode) ||
                (ODBC_USER_DSN != configMode))
            {
                // Only propagate the error if not working with a User DSN.
                throw;
            }
        }
    }
