...
void QSTableMetadataFile::AddColumnMetadata(
    ...
    const simba_uint32* in_columnSize)
{
    ...
    columnMetadata->m_charOrBinarySize = in_columnSize ? *in_columnSize : m_settings->defaultMaxColumnSize;
    ...
    if (sqlTypeMetadata->IsCharacterOrBinaryType())
    {
        ...
        columnMetadata->m_caseSensitive = sqlTypeMetadata->IsAnyCharacterType();
    }
    ...
    const simba_char** in_attributes)
{
    ...
    if (0 == strncmp(in_name, "Column", nameLen))
    {
        ...
        bool hasColumnSize(false);
        simba_uint32 columnSize;
        attrIndex = FindColumnAttribute("ColumnSize", in_attributes);
        if (attrIndex != -1)
        {
            hasColumnSize = true;
            ...
            try
            {
                columnSize = NumberConverter::ConvertStringToUInt32(in_attributes[attrIndex + 1]);
            }
            catch (...)
            {
                SIMBA_ERROR_TRACE_SWALLOW_EXCEPTIONS();
        ...
        it->AddColumnMetadata(columnName, typeString, isUnsigned, hasColumnSize ? &columnSize : NULL);
    }
}
