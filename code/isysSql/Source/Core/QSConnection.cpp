...
// Private =========================================================================================
////////////////////////////////////////////////////////////////////////////////////////////////////
void QSConnection::CleanupCustomProperties()
{
    QSCustomPropertyMap::const_iterator it(m_customConnProperties.begin());
    const QSCustomPropertyMap::const_iterator end(m_customConnProperties.end());
    for (; it != end; ++it)
    {
        delete it->second;
    }

    m_customConnProperties.clear();
}

////////////////////////////////////////////////////////////////////////////////////////////////////
void QSConnection::DoReset()
{
    CleanupCustomProperties();
}
...
void QSConnection::SetConnectionPropertyValues()
{
    ...
    dbfPropInfo.key = QS_DBF_KEY;
    dbfPropInfo.isRequired = true;
    dbfPropInfo.valueType = Variant::TYPE_WSTRING;

    m_settingInfo[DBPROP_INIT_LOCATION] = dbfPropInfo;
#endif
    ...
    SetProperty(DSI_CONN_SERVER_NAME, AttributeData::MakeNewWStringAttributeData(L"Quickstart"));
    SetProperty(DSI_CONN_USER_NAME, AttributeData::MakeNewWStringAttributeData(L"User"));
    SetProperty(DSI_CONN_CURRENT_CATALOG, AttributeData::MakeNewWStringAttributeData(QS_CATALOG));

    // Override default
    SetProperty(DSI_CONN_OUTER_JOINS, AttributeData::MakeNewWStringAttributeData(L"F"));

    SetProperty(
        DSI_CONN_STOP_ON_ERROR,
        AttributeData::MakeNewUInt32AttributeData(DSI_SOE_MULTI_PARAM_SET));
}
